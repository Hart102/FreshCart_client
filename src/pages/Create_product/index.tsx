...
import { fetchProductById, fetchCategories } from "@/components/api/products";

import { openModal } from "@/redux/modal_actions";
import { ResponseModal, Loader } from "@/components/Templates/index";
...
export default function AddProduct() {
  ...
  const fetctSelectedProduct = async () => {
    if (params?.id !== "create") {
      ...
      const { name, price, quantity, status, description, images } = data;
      ...
      <div className="flex flex-col md:flex-row text-sm md:p-0 p-4 justify-center">
        ...
        <form className="flex flex-col gap-8 w-full md:w-4/12 md:px-5 md:pt-8 bg-white">
          <div className="flex flex-col gap-5 [&_span]:text-red-500 [&_span]:text-xs">
            ...
            >
              {params.id == "create" ? "CREATE PRODUCT" : "EDIT PRODUCT"}
            </Button>
          </div>
        </form>
      </div>
